        ...
        <view class="health-grid">
          ...
          <view class="health-item cache-health">
            ...
            <view class="health-details">
              ...
              <text class="health-value" :class="health.cache.status">{{ getCacheStatusText(health.cache.status) }}</text>
          ...
          <view class="health-item api-health">
            ...
            <view class="health-details">
              ...
              <text class="health-value" :class="health.api.status">{{ getAPIStatusText(health.api.status) }}</text>
              <text class="health-metric">响应时间: {{ health.api.responseTime.toFixed(0) }}ms</text>
          ...
          <view class="health-item performance-health">
            ...
            <view class="health-details">
              ...
              <text class="health-metric">平均延迟: {{ performanceMetrics.averageResponseTime }}ms</text>
              <text class="health-metric">API调用减少: {{ performanceMetrics.apiCallReduction }}%</text>
            </view>
          </view>
          ...
          <view class="health-item queue-health">
            ...
            <view class="health-details">
              ...
              <text class="health-value" :class="health.api.queueSize > 10 ? 'degraded' : 'healthy'">
                {{ health.api.queueSize > 0 ? '处理中' : '空闲' }}
              </text>
              <text class="health-metric">队列长度: {{ health.api.queueSize }}</text>
              <text class="health-metric">断路器: {{ health.api.circuitBreakerOpen ? '开启' : '正常' }}</text>
            </view>
          </view>
        </view>
          ...
          <view v-if="performanceImprovement" class="improvement-stats">
            ...
            <view class="improvement-grid">
              <view class="improvement-item positive">
                ...
                <text class="improvement-value">{{ performanceImprovement.timeReduction.toFixed(1) }}%</text>
              </view>
              <view class="improvement-item positive">
                <text class="improvement-label">API调用减少</text>
                <text class="improvement-value">{{ performanceImprovement.apiReduction.toFixed(1) }}%</text>
              </view>
              <view class="improvement-item positive">
                <text class="improvement-label">延迟降低</text>
                <text class="improvement-value">{{ performanceImprovement.latencyReduction.toFixed(1) }}%</text>
              </view>
              <view class="improvement-item positive">
                <text class="improvement-label">可靠性提升</text>
                <text class="improvement-value">{{ performanceImprovement.reliabilityImprovement.toFixed(1) }}%</text>
              </view>
            </view>
          </view>
        </view>
      ...
      <view class="realtime-demo admin-card">
        <view class="demo-header">
          ...
          <view class="demo-controls">
            ...
            <button class="demo-btn secondary" @click="clearDemoResults">
              清空记录
            </button>
          </view>
        </view>
      ...
      <view class="cache-console admin-card">
        <view class="console-header">
          ...
          <view class="console-actions">
            <button class="console-btn" @click="handleWarmupCache">
              预热缓存
            </button>
            <button class="console-btn warning" @click="handleClearCache">
              清空缓存
            </button>
            <button class="console-btn danger" @click="handleResetSystem">
              重置系统
            </button>
          </view>
        </view>
          ...
          <view class="api-queue-status">
            ...
            <view class="queue-details">
              ...
              <view class="queue-item">
                ...
                <text class="queue-value">{{ api.queueStatus.value.processing ? '是' : '否' }}</text>
              </view>
              <view class="queue-item">
...
import { useEnhancedPermissionSystem, enhancedPermissionPresets } from '@/composables/useEnhancedPermissionSystem'
...
const permissionLogs = ref<Array<PermissionCheckResult & {
  userId: string
  resource: string
  action: string
  timestamp: Date
...
const performanceImprovement = computed(() => {
  ...
  return {
    ...
    reliabilityImprovement: ((enhanced.cacheHitRate - baseline.cacheHitRate) / baseline.cacheHitRate) * 100
  }
})
...
const performanceRecommendations = computed(() => {
  ...
  if (health.value.cache.memoryUsage > 100 * 1024 * 1024) { // 100MB
...
const runBaselineTest = async (testCases: Array<{ userId: string, resource: string, action: string }>) => {
...
const runEnhancedTest = async (testCases: Array<{ userId: string, resource: string, action: string }>) => {
  const startTime = performance.now()
  const initialApiCalls = parseInt(performanceMetrics.value.apiCallReduction)
  
  // 使用批量检查进行测试
  const batchResult = await checkPermissionsBatch(testCases.map(tc => ({
    ...tc,
