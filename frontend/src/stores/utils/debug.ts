...
export const useDebugStore = defineStore('debug', () => {
  ...
  const recentActions = computed(() => 
    actionLogs.slice(-50).reverse()
  )
  
  const errorActions = computed(() =>
    actionLogs.filter(log => log.error)
  )
  ...
  const performanceStats = computed(() => {
    ...
    return {
      total: stats.length,
      slowest: stats.reduce((slowest, current) => 
        current.maxTime > (slowest?.maxTime || 0) ? current : slowest, null),
      fastest: stats.reduce((fastest, current) => 
        current.minTime < (fastest?.minTime || Infinity) ? current : fastest, null),
      mostUsed: stats.reduce((mostUsed, current) => 
        current.count > (mostUsed?.count || 0) ? current : mostUsed, null)
    }
  })
  ...
  function logAction(
    ...
    } else if (duration && duration > 100 && ['debug', 'info', 'warn'].includes(config.value.logLevel)) {
