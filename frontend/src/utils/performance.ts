...
export const usePerformanceStore = defineStore('performance', () => {
    ...
    const checkMemory = () => {
      if (typeof window !== 'undefined' && 'performance' in window && (window.performance as any).memory) {
        ...
        if (memoryHistory.length >= 10) {
          ...
          if (trend > 20) { // Growing by more than 20MB in recent samples
  ...
  function recordComponentPerformance(componentName: string, renderTime: number, memoryDelta: number = 0) {
...
export function useMemoryLeakDetector(componentName: string) {
  ...
  let initialMemory = 0
  let timers: number[] = []
  let listeners: Array<{ element: any; event: string; handler: Function }> = []
  ...
  onUnmounted(() => {
    ...
    if (initialMemory > 0 && typeof window !== 'undefined' && 'performance' in window && (window.performance as any).memory) {
      const finalMemory = (window.performance as any).memory.usedJSHeapSize
      const memoryDelta = finalMemory - initialMemory
      
      if (memoryDelta > 1024 * 1024) { // More than 1MB
        performanceStore.addWarning('potential-memory-leak', `组件 ${componentName} 可能存在内存泄漏 (+${Math.round(memoryDelta / 1024 / 1024)}MB)`)
      }
    }
  })
