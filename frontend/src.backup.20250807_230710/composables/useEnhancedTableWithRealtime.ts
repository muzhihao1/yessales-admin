...
import { useTableEnhancements, type VirtualScrollingConfig, type TableItem } from './useTableEnhancements'
import { useRealTimeUpdates, type RealTimeUpdateOptions, type DataItem, realtimeUpdatePresets } from './useRealTimeUpdates'
...
export function useEnhancedTableWithRealtime<T extends TableItem & DataItem>(
  ...
  // 初始化表格增强功能
  const tableEnhancements = useTableEnhancements<T>(
    { pageSize: 20 },
    options.virtualConfig
  )

  // 初始化实时更新功能
  const realtimeUpdates = useRealTimeUpdates<T>(
    options.dataKey,
    realtimeConfig
  )
