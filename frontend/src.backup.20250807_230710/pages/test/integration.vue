        ...
        <view class="component-showcase">
          <view class="showcase-item">
            ...
            <LoadingSkeleton 
              variant="product" 
              :item-count="3"
              custom-class="showcase-skeleton"
            />
          </view>
...
const completedTests = computed(() => allTests.value.filter(t => t.status !== 'pending' && t.status !== 'running').length);
...
const allTestsPassed = computed(() => completedTests.value === totalTests.value && failedTests.value === 0);
