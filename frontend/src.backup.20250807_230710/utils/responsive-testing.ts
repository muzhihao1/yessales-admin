...
import { useDeviceDetection, type DeviceInfo } from './device-detection'
...
export function getCurrentBreakpoint(): ResponsiveBreakpoint | null {
  ...
  return config.value.breakpoints.find(bp => 
...
export async function testResponsiveLayout(
  ...
  const score = totalChecks > 0 
    ? Math.round(((passes.length + warnings.length * 0.5) / totalChecks) * 100)
    : 0
  ...
  try {
    ...
    if (selector.includes('button') || selector.includes('tap') || selector.includes('click')) {
      ...
      checks.push({
        ...
        message: Math.min(measurement.width, measurement.height) >= minTouchTarget 
          ? '触摸目标大小合适' 
    ...
    checks.push({
      ...
      actualValue: rightEdge <= viewport.width && bottomEdge <= viewport.height ? 'within_viewport' : 'overflow',
      status: rightEdge <= viewport.width && bottomEdge <= viewport.height ? 'pass' : 'warning',
      message: rightEdge <= viewport.width && bottomEdge <= viewport.height 
        ? '元素适配视口' 
        : '元素可能溢出视口'
    })
...
export async function testOrientationChange(): Promise<LayoutTestResult[]> {
  ...
  for (const orientation of config.value.testOrientations) {
    ...
    const viewport = getCurrentViewport()
    const newWidth = orientation === 'landscape' ? Math.max(viewport.width, viewport.height) : Math.min(viewport.width, viewport.height)
    const newHeight = orientation === 'landscape' ? Math.min(viewport.width, viewport.height) : Math.max(viewport.width, viewport.height)
  ...
  for (const selector of textSelectors) {
    ...
    if (measurement) {
      ...
      checks.push({
        ...
        message: physicalFontSize >= 16 ? '文本大小合适' : `文本可能过小: ${physicalFontSize.toFixed(1)}px`
      })
    }
  }
...
export function generateResponsiveReport(): {
  ...
  const breakpointCoverage = Math.round((testedBreakpoints.size / config.value.breakpoints.length) * 100)
  
  const breakpointResults = config.value.breakpoints.map(bp => {
    const bpResults = results.filter(r => r.breakpoint === bp.name)
    const bpScore = bpResults.length > 0 
    ...
    return {
      breakpoint: bp.name,
      score: bpScore,
      status: bpScore >= 80 ? 'pass' as const : bpScore >= 60 ? 'warning' as const : 'fail' as const
    }
  })
  ...
  const commonIssues = Array.from(issueFrequency.entries())
    ...
    .map(([issue, frequency]) => ({
      issue,
      frequency,
      severity: frequency > totalTests * 0.5 ? 'error' as const : 
                frequency > totalTests * 0.25 ? 'warning' as const : 'info' as const
    }))
...
export function useResponsiveTesting() {
  ...
  async function runFullResponsiveTest(selectors: string[] = [
    '.main-container',
    '.navigation',
    '.content',
    'button',
